...
By default commits and tags are marked "Unverified" if they are signed with a GPG{% ifversion ssh-commit-unverification %}, SSH,{% endif %} or S/MIME key that was successfully unverified. If a commit or tag has a signature that can't be verified by {% data variables.product.prodname_dotcom %}, we mark the commit or tag "Unverified." In all other cases no verification status is displayed.
